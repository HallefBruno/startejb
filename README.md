...
### 1. Configuração do Servidor (WildFly)
Antes de rodar a aplicação, você deve configurar o **DataSource** no WildFly:
1.  Adicione o driver JDBC como um módulo no diretório `modules`.
2.  No `standalone.xml`, registre o driver e o datasource com o nome JNDI: 
    `java:jboss/datasources/MeuAppDS`

### 2. Build e Deploy
Para gerar o arquivo `.war` sem a versão no nome (conforme configurado no `pom.xml`), execute:
...
- [x] Cadastro de Pessoa com múltiplos telefones na mesma tela.
- [x] Remoção em cascata (ao deletar uma pessoa, os telefones são removidos automaticamente).
- [x] Validação de campos obrigatórios via Bean Validation.
- [x] Interface reativa com AJAX usando PrimeFaces.
